...
# ---------------------------------------------------------------------------
# Block 15F-B checkpoint: presentation-dashboard command asset
# ---------------------------------------------------------------------------


def test_095_presentation_dashboard_command_discoverable():
    result = lookup_ai_asset(asset_id="claude_command:presentation-dashboard")
    assert result["found"] is True
    assert result["asset_type"] == "claude_command"
    assert result["declared_in"] == ".claude/commands/presentation-dashboard.md"


def test_096_presentation_dashboard_provenance_is_repository_declared_not_verified():
    result = lookup_ai_asset(asset_id="claude_command:presentation-dashboard")
    assert result["provenance"]["tier"] == "repository_declared"
    assert "verified" not in result["provenance"]["tier"]
    assert "authenticated" not in result["provenance"]["tier"]
...
def test_097_no_gateway_tool_or_identity_agent_added_for_presentation_dashboard():
    assert lookup_ai_asset(asset_id="gateway_tool:presentation_dashboard")["found"] is False
    assert lookup_ai_asset(asset_id="gateway_tool:render_presentation_dashboard")["found"] is False
    assert lookup_ai_asset(asset_id="identity_agent:presentation_dashboard_agent")["found"] is False


def test_098_no_claude_subagent_or_skill_added_for_presentation_dashboard():
    assert lookup_ai_asset(asset_id="claude_subagent:presentation-dashboard")["found"] is False
    assert lookup_ai_asset(asset_id="claude_skill:presentation-dashboard")["found"] is False


def test_099_no_duplicate_asset_ids_after_block_15f_b_addition():
    listing = list_ai_assets()
    ids = [asset["asset_id"] for asset in listing["assets"]]
    assert len(ids) == len(set(ids))
