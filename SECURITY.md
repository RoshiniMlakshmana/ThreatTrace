...
- The tool adapter registry (`adapters.bug_bounty_{http,nmap,nuclei,zap,burp}`) is a fixed, hardcoded Python mapping — there is no code path by which a `tool_id` string selects an arbitrary binary.
